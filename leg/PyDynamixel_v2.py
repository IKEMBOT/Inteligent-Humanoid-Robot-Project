...
if PROTOCOL_VERSION == 1:
    ADDR_XL_TORQUE_ENABLE       = 24    # Address for torque enable
    ADDR_XL_PRESENT_POSITION    = 37    # Address for the current position
    ADDR_XL_GOAL_POSITION       = 30    # Address for goal position
    LEN_XL_GOAL_POSITION        = 2     # XL's have different message lengths in Protocol 1 vs. 2
    ...
    ADDR_XL_GOAL_POSITION       = 30   # Address for goal position
    ADDR_XL_SPEED               = 32 
    LEN_XL_GOAL_POSITION        = 2
    LEN_XL_PRESENT_POSITION     = 2
...
class DxlComm(object):
    ...
    def __init__(self, port="/dev/ttyUSB0", baudrate=None, baudnum=None):
        ...
        if PROTOCOL_VERSION == 2:
            ...
            self.group_sync_read = GroupSyncRead(self.port_handler, self.packet_handler, ADDR_XL_PRESENT_POSITION, LEN_XL_PRESENT_POSITION)
        self.group_sync_write = GroupSyncWrite(self.port_handler, self.packet_handler, ADDR_XL_GOAL_POSITION, LEN_XL_GOAL_POSITION)
    ...
    def _sync_write(self, servos=None, values=None, radian=False):
        ...
        if servos != None:
            ...
            for k, goal_value in values.items():
                ...
                param_goal_position = [DXL_LOBYTE(DXL_LOWORD(goal_value)), DXL_HIBYTE(DXL_LOWORD(goal_value))]
    ...
    def get_angles(self, radian=False):
        ...
        if PROTOCOL_VERSION == 2:
            servos_angles = self._sync_read(ADDR_XL_PRESENT_POSITION, LEN_XL_PRESENT_POSITION, radian=radian)
        ...
        if PROTOCOL_VERSION == 2:
            ...
            for i, servo_id in enumerate(self.joint_ids):
                ...
                if dxl_getdata_result != True:
                    ...
                    if radian:
                        present_angle = (radians(300)*dxl_present_position)/1024.0
                    else:
                        present_angle = (300*dxl_present_position)/1024.0
    ...
    def enable_torques(self):
        ...
        in this port.
        '''
        self.packet_handler.write1ByteTxRx(self.port_handler, BROADCAST_ID, ADDR_XL_TORQUE_ENABLE, 1)

    def disable_torques(self):
        ''' Disables torque for all motors connected
        to this port
        '''
        self.packet_handler.write1ByteTxRx(self.port_handler, BROADCAST_ID, ADDR_XL_TORQUE_ENABLE, 0)
    ...
    def set_goal_value(self, angle, radian=False):
        ...
        if radian:
            ...
            goal_value = int(1024*angle/radians(300))
        else:
            # goal_value = int(2048.0*angle/180)
            goal_value = int(1024*angle/300)
        return goal_value
...
class Joint(object):
    ...
    def set_goal_value(self, angle, radian=False):
        ...
        if radian:
            self.goal_value = int(1024.0*angle/radians(300)) + self.center_value
        else:
            self.goal_value = int(1024.0*angle/300) + self.center_value
        self.changed = True
    ...
    def send_angle(self, angle, radian=False):
        ...
        dxl_comm_result, dxl_error = self.packet_handler.write4ByteTxRx(self.port_handler, self.servo_id, ADDR_XL_GOAL_POSITION, self.goal_value)
    ...
    def get_angle(self, radian=False):
        ...
        '''
        if PROTOCOL_VERSION == 1:
            self.curr_value, dxl_comm_result, dxl_error = self.packet_handler.read2ByteTxRx(self.port_handler, self.servo_id, ADDR_XL_PRESENT_POSITION)
        elif PROTOCOL_VERSION == 2:
            self.curr_value, dxl_comm_result, dxl_error = self.packet_handler.read4ByteTxRx(self.port_handler, self.servo_id, ADDR_XL_PRESENT_POSITION)
        ...
        if radian:
            self.curr_angle = radians(300)*float(self.curr_value)/1024.0
        else:
            self.curr_angle = 300*float(self.curr_value)/1024.0
    ...
    def enable_torque(self):
        ...
        '''
        dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, self.servo_id, ADDR_XL_TORQUE_ENABLE, 1)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % self.packet_handler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % self.packet_handler.getRxPacketError(dxl_error))
    ...
    def disable_torque(self):
        ...
        Usage:
            self.disable_torque()
        '''
        dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, self.servo_id, ADDR_XL_TORQUE_ENABLE, 0)
